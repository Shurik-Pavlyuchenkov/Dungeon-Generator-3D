...
public class Generator : MonoBehaviour
{
    ...
    class Room
    {
        ...
        public static bool Intersect(Room a, Room b)    //Проверка на пересечение комнат
        {
    ...
    [SerializeField]
    Vector3Int size;          //Размер карты 
    [SerializeField]
    int roomCount;            //Количество комнат 
    [SerializeField]
    Vector3Int roomMaxSize;   //Максимальный размер комнат 
    [SerializeField]
    GameObject cubePrefab;    //Модель куба
    [SerializeField]
    Material greenMaterial;   //Материал для комнат 
    [SerializeField]
    Material yellowMaterial;  //Материал для коридоров 
    [SerializeField]
    Material blueMaterial;    //Материал для лестниц
    ...
    void PlaceRooms()
    {
        for (int i = 0; i < roomCount; i++)
        {
            ...
            Vector3Int roomSize = new Vector3Int(
                ran.Next(1, roomMaxSize.x + 1),
                ran.Next(1, roomMaxSize.y + 1),
                ran.Next(1, roomMaxSize.z + 1)
            );
    ...
    void PathfindHallways()
    {
        ...
        foreach (var edge in selectedEdges)
        {
            ...
            var path = aStar.FindPath(startPos, endPos, (DungeonPathfinder3D.Node a, DungeonPathfinder3D.Node b) => {
                ...
                {
                    //лестница
                    if ((grid[a.Position] != CellType.None && grid[a.Position] != CellType.Hallway) || (grid[b.Position] != CellType.None && grid[b.Position] != CellType.Hallway)) return pathCost;
    ...
    {
        PlaceCube(location, new Vector3Int(1, 1, 1), yellowMaterial);
    }
}
